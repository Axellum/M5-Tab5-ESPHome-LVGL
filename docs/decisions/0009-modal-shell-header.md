...
3. **One scrim.** `modal_scrim.yaml` takes a `scrim_opa` var — `100` for top-level popups, `60` for `cal_day_popup` (it stacks on an already-dimmed screen). No inline scrim anywhere; `style_modal_overlay` (72 %) is deleted.
   - *Update 2026-09-26 (resource audit, lot 6 — D7 trial, kept by Axel):* top-level scrims went from 85 % to **100 %, square corners** (`radius: 0`; a button gets a theme radius, and a rounded scrim no longer "covers" the screen for LVGL). LVGL then skips the hidden dashboard entirely.
