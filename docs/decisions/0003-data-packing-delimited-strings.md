...
- **Update 2026-09-08:** the last per-item push, the 9-bar rain chart (`tab5_maj_pluie_1h`, nine calls plus nine 50 ms delays per refresh), got its bulk twin `tab5_maj_pluie_1h_bulk` (`idx|intensity;…`, 256-byte buffer). The per-bar service was removed the same day, once the production automation, the public example and the demo mode were on the bulk.
