...
- **Update 2026-09-08 (audit lot (e)):** `tab5_custom.cpp` is split into nine units (`tab5_text/forecast/central/services/assist/cards/console/anim/calendar.cpp`, 164 to 741 lines each); `tab5_custom.h` stays the single public header, so the rule reads exactly as before — call a function declared there. `tab5_internal.h` holds the six helpers shared between units.
- **Update 2026-09-08:** for the HA contract it *is* now linted. `tools/check_tab5_code_rules.py` (run by `pytest`, so by CI) fails on any `lv_*` call in `tab5-api-logic.yaml` other than the read-only `lv_obj_has_flag`, on any raw `sprintf` anywhere in `Tab5/`, and on any `globals:` entry nobody references. The last inline LVGL blocks of the contract (shutter, Météo-France vigilance, 1 h rain bars, snow icon, climate target, assistant image hint) moved to `tab5_custom.cpp` that day.
