...
- Production-only HA files (`automations_tab5.yaml`, `scripts_tab5.yaml`, `template_sensors_meteo_tab5.yaml`) stay gitignored and outside this pipeline. They are mirrored by hand into the `*_examples*` files, which drifts easily (see `AGENTS.md`).
