...
- Since 2026-09-26 a second job, `build-min`, compiles the same configuration with the floor itself, read from `min_version:` in `tab5-ha-hmi.yaml` (the only source of the floor). `build` watches the newest ESPHome, `build-min` the oldest one the project still claims to support: an option that appeared after the floor no longer slips through. It has its own ccache key per version and no artifact. Since the evening of 2026-09-26 it is a required check of `main`, like `build` (skipped, so passing, when the firmware does not change).
