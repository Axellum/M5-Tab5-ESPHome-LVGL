...
- `tab5_calendar.cpp` — `cal_grid_build()` (the 42 grid cells), `cal_store_month_data()`, `cal_render_month()`, `cal_render_day_detail()` (cache mensuel `static`)
- `tab5_journal.cpp` — boot and outage journal (2026-09-26): `journal_log_message()` (`logger: on_message`) keeps errors, and warnings while HA is not connected, in `.noinit` RAM with an NVS copy once Wi-Fi has been missing for 90 s; sent to HA as `esphome.tab5_journal` on reconnection (see `docs/debugging.md`)

`tab5_internal.h` declares the 13 helpers shared *between* units (`normalize_text_utf8()`, `set_label_text_utf8()`, `transition_widgets()`, `close_popup_if_open()`…); it is not part of the YAML contract. Since 2026-09-25 (audit lot 8d) **`tab5_custom.h` declares only functions a YAML lambda calls** (80 on 2026-09-26, lot 7, each one checked against the YAML) — a helper used by one unit only is `static` there, one shared by several units goes to `tab5_internal.h`, and the day/month names live in `tab5_core.cpp` (`fr_day_short_utf8()`, `fr_day_long_utf8()`, `fr_month_long_utf8()`, `clock_month_short_utf8()`, `fr_capitalized()`). **Rule: sensors/services should only read HA state and call these C++ functions — never manipulate `lv_obj_*` directly from a `sensor:`/`text_sensor:`/`api: services:` lambda.** Adding a feature = one `update_*_ui()` in the unit that owns the responsibility + its declaration in `tab5_custom.h`.

**Architecture `CentralPanelCtx`** (depuis refacto 26/07) : le struct `CentralPanelCtx` regroupe les 8 wrappers LVGL de la carte centrale (+ `page_title_sub`, la ligne chapeau du titre de page prévisions) + 7 flags d'activité + `current_panel`. Les pointeurs sont initialisés **une fois au boot** (`on_boot` dans `tab5-ha-hmi.yaml`) ; les 7 flags et `current_panel` n'existent **que** dans `g_central_ctx` : les lambdas lisent et écrivent `g_central_ctx.has_rain`, `g_central_ctx.current_panel`… directement (source unique depuis le lot 7 de l'audit du 26/09/2026 ; avant, 8 globals ESPHome les doublaient et chaque script les recopiait avant et après chaque appel C++). Globals C++ : `g_central_ctx`, `g_day_slots[5]`, `g_hour_slots[5]`.
...
| `tab5_assist_reponse` | texte, image_url (strings) | Réponse « riche » du moteur pour le popup Assistant : Markdown (tableaux alignés approximativement — police proportionnelle depuis le 26/09/2026 —, gras, code) + URL d'image optionnelle téléchargée à la demande (`online_image`). `image_url` vide = zone image masquée. Ouvre le popup automatiquement |
