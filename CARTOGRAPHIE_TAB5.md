...
graph TD
    ...
    subgraph PKG["Packages ESPHome (Tab5/*.yaml) — 12 packages"]
        ...
        SENSO["tab5-sensors-domotique.yaml<br/>401 lignes<br/>plantes/lumières (+brightness live)/PC/températures/batterie/audio"]
        ...
        SCR["tab5-scripts.yaml<br/>1145 lignes<br/>script: debounces + vocal (+ wake_word_dispatch) + rotateur/dismiss + volet + popups + jeux open/close + init ModalRegistry"]
...
règles calendrier ouverture/fermeture, snooze, liste RDV, rendu LVGL<br/>lit cal_jours_data[] de tab5_custom.h — aucun id() ESPHome, aucun réseau"]
    ...
    subgraph CPP["C++ HMI (esphome: includes:)"]
        ...
        CFILE["tab5_custom.cpp (3169L)<br/>logique LVGL HMI non-triviale"]
...
- **`tab5_custom.cpp` (3169 lignes au 08/09/2026)** — 7 responsabilités (UTF-8/texte, météo, carte centrale, assistant/markdown, calendrier, console, animations + horloge) ; découpage en unités de compilation recommandé par l'audit du 06/09 — `esphome: includes:` accepte plusieurs `.cpp`, comme pour les jeux.
