...
| `tab5_journal.cpp` | 286 | Journal des démarrages et des coupures (26/09/2026) : `logger: on_message` → `journal_log_message()` garde les erreurs (dont `esp-idf` : pilote ESP-Hosted du C6) et, HA déconnecté, les avertissements, en `.noinit` (32 lignes, survit aux resets logiciels et aux plantages, rapport `esp32.crash` d'ESPHome compris) ; `journal_tick()` (interval 30 s) copie en NVS après 2 min sans HA ; script `tab5_journal_envoi` (connexion de HA) → événement `esphome.tab5_journal` → garde (e) de `packages/tab5_health.yaml` | voir `tab5_custom.h` |
