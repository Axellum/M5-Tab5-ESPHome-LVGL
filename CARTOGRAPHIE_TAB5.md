...
| `game_common.h` | 300 | Header-only partagé par les **8 consoles** (ADR-0014) : `clampf()`, `xorshift32_next()` (l'état reste dans chaque jeu), `mk_rect()`/`mk_label()`, `show()`/`set_bg()`/`set_border()`/`set_text_if()`/`set_text_color_if()` (sur-ensemble des variantes : gardes nulles, opacité par défaut), `NvsSlot<T>` (clé + magic, `load()`/`save()`/`ready()`) et, depuis le lot 8f (25/09/2026), les mécanismes que 2 à 4 consoles recopiaient : `timer_period_sync()` (tick adaptatif), `topn_insert<>()` (classement), `tilt_calibrate()`/`tilt_smooth()` (inclinaison), `accel_delta_norm()`/`shake_fire()` (secousse) — seuils, périodes et gardes restent dans chaque jeu. Depuis le lot 4 de l'audit des ressources (26/09/2026) : `game_mem_new<T>(MemPref)` / `game_mem_free()` (état d'un jeu pris à l'ouverture, rendu à la fermeture) et `ui_destroy()` (objets LVGL du jeu détruits, conteneurs YAML gardés) — jeu fermé, rien ne reste réservé. Ne dépend pas du HMI ; un jeu peut redéfinir un helper dans son namespace (`Go::set_bg`) | — |
| `tab5_text.cpp` | 180 | UTF-8 / mojibake, store local des alertes rejetées, `set_label_text_utf8()` (les dates et libellés de jours/mois sont passés dans `tab5_core.cpp`) | voir `tab5_custom.h` |
| `tab5_core.h/.cpp` | 243 | Logique **pure** partagée HMI + réveil (25/09/2026, lot 8b) : `DayForecastData`/`HourForecastData`, `cal_jours_data[]`/`cal_jours_anchor_day` (déclarés ; définis dans `tab5_custom.cpp`), `local_day_from_offset()`/`local_day_number_today()` (seule arithmétique de dates du projet, immunisée aux bascules d'heure), `fr_day_long_utf8()`/`fr_month_long_utf8()`, titres de jour, `cal_is_early_shift()`. L'heure passe par `tab5_time_source` (défaut `time`) pour que les tests hôte simulent « maintenant ». Inclus par `tab5_custom.h` | `local_day_from_offset()`, `local_day_number_today()` |
| `tab5_forecast.cpp` | 379 | `update_meteo_icon()`, couleurs température/humidité, `parse_and_update_heures_bulk()`/`_jours_bulk()`, `refresh_daily_forecast()`/`refresh_hourly_forecast()` | voir `tab5_custom.h` |
| `tab5_central.cpp` | 824 | carte centrale (rotateur, `parse_and_update_ha_alerts_bulk()`, `update_info_text_ui()`, titre de page), `handle_swipe_gesture()`/`reset_forecast_to_main_page()`, `show_temporary_planning()`, `show_vocal_response_ui()` | voir `tab5_custom.h` |
| `tab5_services.cpp` | 361 | logique des services HA : `update_volet_ui()`, `parse_and_update_vigilance()`, `update_rain_bar_ui()`, `update_rain_predict_icon_ui()`, `update_clim_from_ha_ui()`, `update_planning_text_ui()` | voir `tab5_custom.h` |
| `tab5_assist.cpp` | 291 | `format_assist_markdown()`, `assist_set_pipeline_state()`, `assist_image_state_ui()`, `WakeWord::decide()` | voir `tab5_custom.h` |
| `tab5_cards.cpp` | 341 | `update_light_card_ui()`, popup lumière, `update_clim_target_ui()`, `sort_and_update_moisture_slots()`, `update_pots_popup_moisture_ui()`/`update_pot_metric_ui()`, `update_temp_ui()` | voir `tab5_custom.h` |
| `tab5_console.cpp` | 192 | ligne status, `ui_sync_volume_widgets()`, `update_console_diagnostics_ui()` | voir `tab5_custom.h` |
| `tab5_anim.cpp` | 646 | `transition_widgets()`, `animate_popup_open()`/`_close()`, `animate_swipe_horizontal()`, `animate_alert_enter()`, `ui_idle_ms()`/`close_popup_if_open()`, `animate_icon_roll_in()`, `layout_clock_roller()`/`update_clock_date_ui()`, `setup_button_press_animation()`/`highlight_button_border()` | voir `tab5_custom.h` |
| `tab5_calendar.cpp` | 317 | `cal_store_month_data()`, `cal_render_month()`, `cal_render_day_detail()` (cache mensuel `static`) | voir `tab5_custom.h` |
...
|---|---|---|---|
| `climate_card.yaml` | 127 | Carte clim compacte (dashboard principal) | — |
...
| `forecast_daily.yaml` | 256 | 5 cartes prévisions journalières (fenêtre glissante sur 15 jours) | Onglets titre/température factorisés via `forecast_day_title_tab.yaml`/`forecast_day_temp_tab.yaml` ; le "corps sombre + action" par carte reste dupliqué 5× (actions HA différentes par carte, cf. §4) |
| `forecast_day_title_tab.yaml` | 14 | Template onglet titre jour | Réutilisé 5× |
| `forecast_day_temp_tab.yaml` | 41 | Template onglet température jour | Réutilisé 5× |
...
| `switches_card.yaml` | 209 | Cartes switches (PC, volet, lumières) | Onglets titre/état factorisés via `switch_card_title_tab.yaml`/`switch_card_state_tab.yaml` ; actions par carte laissées en clair (logique différente par carte) |
...
| `light_popup.yaml` | 367 | Popup contrôle lumière plein écran (1250×690) : sélecteur 3 lumières + On/Off + Tout éteindre, arc luminosité avec % live + raccourcis, 3 blancs + 12 pastilles couleur | Pastilles factorisées via `light_color_preset_btn.yaml` (12 instances) ; ouverture/sélection via `script.tab5_light_popup_show` |
| `light_color_preset_btn.yaml` | 24 | Template pastille couleur ronde 78 px | Réutilisé 12× |
| `tv_remote_popup.yaml` | 521 | Popup télécommande TV Samsung plein écran (1250×690, tokens ADR-0009) : power, pad, volume, chaînes, rangée lecture — `remote.send_command`/`remote.toggle` sur `${entity_tv_remote}` ; ouvert par `btn_control_tv` ou long-press carte PC | — |
...
| `alarm_step_script_btn.yaml` | 29 | Template bouton à pas du réveil qui appelle un script avec `delta` (heure, bornes, mélodie) (lot 8e) | Réutilisé 10× par `alarm_popup.yaml` |
