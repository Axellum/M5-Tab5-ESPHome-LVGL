...
| `game_common.h` | 233 | Header-only partagé par les **8 consoles** (ADR-0014) : `clampf()`, `xorshift32_next()` (l'état reste dans chaque jeu), `mk_rect()`/`mk_label()`, `show()`/`set_bg()`/`set_border()`/`set_text_if()` (sur-ensemble des variantes : gardes nulles, opacité par défaut), `NvsSlot<T>` (clé + magic, `load()`/`save()`/`ready()`) et, depuis le lot 8f (25/09/2026), les mécanismes que 2 à 4 consoles recopiaient : `timer_period_sync()` (tick adaptatif), `topn_insert<>()` (classement), `tilt_calibrate()`/`tilt_smooth()` (inclinaison), `accel_delta_norm()`/`shake_fire()` (secousse) — seuils, périodes et gardes restent dans chaque jeu. Ne dépend pas du HMI ; un jeu peut redéfinir un helper dans son namespace (`Go::set_bg`) | — |
