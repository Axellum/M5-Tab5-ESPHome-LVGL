...
| Fichier | Rôle |
|---|---|
| `.github/workflows/esphome-tab5.yml` | CI GitHub Actions (PR + push sur `main`) : job `python` (pre-commit, pytest, moteur Go C++ par g++, dry-run démo) et job `build` (`secrets.yaml` factice, `esphome/build-action@v8.1.0` sur `latest`, ccache conservé entre runs, sauté si seuls des `.md` changent) et job `build-min` (même compilation avec la version lue dans `min_version:` de `tab5-ha-hmi.yaml`, cache ccache à part, sans artefact) ; artefact `tab5-firmware` sur `main`. `python` et `build` sont requis |
