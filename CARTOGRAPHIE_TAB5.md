...
| `game_common.h` | 159 | Header-only partagé par les **8 consoles** (ADR-0014) : `clampf()`, `xorshift32_next()` (l'état reste dans chaque jeu), `mk_rect()`/`mk_label()`, `show()`/`set_bg()`/`set_border()`/`set_text_if()` (sur-ensemble des variantes : gardes nulles, opacité par défaut), `NvsSlot<T>` (clé + magic, `load()`/`save()`/`ready()`). Ne dépend pas du HMI ; un jeu peut redéfinir un helper dans son namespace (`Go::set_bg`) | — |
