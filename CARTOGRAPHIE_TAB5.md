...
| `tab5_forecast.cpp` | 374 | `update_meteo_icon()`, couleurs température/humidité, `parse_and_update_heures_bulk()`/`_jours_bulk()`, `refresh_daily_forecast()`/`refresh_hourly_forecast()` | voir `tab5_custom.h` |
| `tab5_central.cpp` | 827 | carte centrale (rotateur, `parse_and_update_ha_alerts_bulk()`, `update_info_text_ui()`, titre de page), `handle_swipe_gesture()`/`reset_forecast_to_main_page()`, `show_temporary_planning()`, `show_vocal_response_ui()` | voir `tab5_custom.h` |
