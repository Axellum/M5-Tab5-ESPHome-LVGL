...
`Généré le 2026-07-06` · `maj: 2026-09-25` (comptes de lignes recalculés par `python tools/cartographie_counts.py --write`, vérifiés par pytest à 20 % près ; seule copie — celle de `contexte_ia/02_Hardware/` n'est plus qu'un renvoi ici) · Sources vérifiées directement dans le code (`00ProjetTab/`), croisées avec `Tab5/README.md` (réécrit le 05/07/2026 contre le firmware réel), `contexte_ia/04_Projets/etat_tab5.md` et `contexte_ia/02_Hardware/rules_esphome.md`. Aucun fait ci-dessous n'est tiré d'une supposition — chaque ligne cite le fichier source lu.
...
|---|---|---|---|---|
| `tab5-hardware.yaml` | 363 | Bas niveau : display MIPI-DSI + tactile, partie commune aux 3 révisions du Tab5 (broches, dimensions, calibration, réveil au toucher ; id `tab5_display` et `touch` étendus par `ecran-*.yaml`), DAC ES8388 (plateforme `audio_dac:`) / ADC micro ES7210 (`audio_adc:`), I2S haut-parleur/micro, expander GPIO PI4IOE5V6408, `esp32_hosted` (co-proc WiFi ESP32-C6 via SDIO 20 MHz), `media_player`, `ota:` (la pile vocale est dans `tab5-assist.yaml` depuis le lot 8c) | Hardware, audio, OTA | composants natifs ESPHome ≥ 2026.8.1 (plus d'`external_components` depuis 2026.7.0) |
| `ecran-st7123.yaml` | 22 | Révision ST7123 (défaut, la tablette d'Axel) : `!extend tab5_display` → `mipi_dsi` modèle `M5STACK-TAB5-ST7123` ; `!extend touch` → `st7123`, `update_interval: 100ms` | Hardware | `tab5-hardware.yaml` (id étendus) |
| `ecran-st7121.yaml` | 22 | Révision ST7121 (depuis le 28/04/2026) : modèle `M5STACK-TAB5-ST7121`, tactile `st7123` (hypothèse : pas de pilote ST7121 dans ESPHome). Compilé par la CI (`build-revisions`), jamais testé sur appareil | Hardware | idem |
| `ecran-ili9881c.yaml` | 21 | Tab5 d'origine (avant le 14/10/2025) : modèle `M5STACK-TAB5`, tactile `gt911` (`update_interval: never`), repris de devices.esphome.io. Compilé par la CI, jamais testé sur appareil | Hardware | idem |
| `tab5-sensors-diagnostics.yaml` | 381 | `wifi:`, switchs d'alim GPIO (WiFi/USB/5V ext/antenne), statut API HA, IP/SSID, uptime, RSSI, température coeur, RAM libre/loop time (`debug`), durée max des images LVGL (`on_draw_start/end`) et pile libre min de la boucle, select antenne, horloge SNTP, `interval:` icône WiFi 5s + console 2s | Réseau WiFi, alimentation, diagnostics système | `tab5_custom.h` (`update_console_*`, `update_clock_date_ui`, `is_console_layer_visible`) |
| `tab5-sensors-domotique.yaml` | 232 | Miroirs d'entités HA : humidité 5 plantes (triées dynamiquement + 5 cartes fixes du popup détails), 20 capteurs détails pots (`pot*_ec/lux/temp/bat` — EC, éclairement, température, batterie — venus de `pot_sensors.yaml`, package paramétré inclus 5× par un `packages:` imbriqué), lumières chambre/salon/LED (+ 3 capteurs `attribute: brightness` pour la synchro live de l'arc du popup lumière), présence PC, batterie téléphone, températures/humidité salon/chambre/serre, audio (ampli, jack, wake word) | Capteurs domotique et miroirs d'entités HA | `tab5_custom.h` (`get_temperature_color`, `get_humidity_color`, `get_battery_color`, `sort_and_update_moisture_slots`, `update_pots_popup_moisture_ui`, `update_pot_metric_ui`, `update_light_card_ui`, `update_temp_ui`, `sync_light_popup_brightness`) |
...
| `tab5-api-logic.yaml` | 503 | Le contrat réel avec HA : bloc `api: services:` (16 services : `tab5_maj_pluie_1h_bulk` remplace `tab5_maj_pluie_1h` depuis le 08/09/2026). Chaque service `tab5_maj_*` reçoit un payload d'une automation HA et appelle une fonction C++ (`tab5_services.cpp`, déclarée dans `tab5_custom.h`) via lambda (pattern : sync `g_central_ctx` ← globals, appel C++, write-back) | Contrat API HA↔Tab5 (clim, volet, planning, alertes météo France, probabilités UV/gel/neige, prévisions bulk, pluie 1h, panneau info, réponse vocale, alertes HA bulk, calendrier mois/jour) | `tab5_custom.h/.cpp`, IDs LVGL définis dans `tab5-lvgl.yaml`/`ui_components/*.yaml` |
| `tab5-styles.yaml` | 643 | Thème "Dark Mode Slate" (glassmorphism) : tokens `color:`, déclarations `font:` (Roboto + MDI + police météo custom), `lvgl: style_definitions:` | Palette visuelle, typographie, styles réutilisables | Polices `Tab5/materialdesignicons-webfont.ttf`, `Tab5/IconeMeteo.ttf` |
| `tab5-globals.yaml` | 204 | Tout l'état partagé entre fichiers (`globals:`) + l'`interval: 8s` qui fait tourner la carte centrale (planning/pluie/alertes/info + jusqu'à 4 bandeaux HA, actif seulement sur la fenêtre prévisions par défaut). Contenu et panneau courant de la carte centrale : **aucun global**, seule source `g_central_ctx` (lot 7, 26/09/2026) | État global partagé, rotateur carte centrale | `tab5_anim.cpp` (`transition_widgets()`), `g_central_ctx` |
| `tab5-scripts.yaml` | 485 | Scripts transverses : init du registre des modales (`tab5_modal_registry_init`), volume (`tab5_volume_apply`, point d'entrée unique), debounces (volume 150 ms, luminosité 200 ms, clim 250 ms), recoloration clim, rotateur central + dismiss (info, alertes HA paramétré slot 0-3), volet (fin de mouvement, feedback stop, tap de la carte `tab5_volet_tap`), popup lumière (`tab5_light_popup_show`), touches de la télécommande TV (`tab5_tv_key(touche)`, lot 7), `interval:` 1 s de retour à l'accueil. L'affichage temporaire du planning est en C++ (`show_temporary_planning()`) | Séquences temporisées, rotateur, retour accueil | `globals:`, `tab5_custom.h` (unités `tab5_*.cpp`), `g_central_ctx` |
| `tab5-arcade.yaml` | 203 | Scripts des jeux (lot 8c) : `tab5_games_close_all` (liste lue dans `GameRegistry::kGames`), `tab5_<jeu>_open` ×8 (injection des pointeurs LVGL et des polices), `tab5_arcade_open` | Arcade | `tab5_registry.*`, `*_game.h` |
| `tab5-calendar.yaml` | 175 | Scripts du popup calendrier (lot 8c) : `tab5_calendar_open`, `tab5_cal_render`, `tab5_cal_prev/next/today`, `tab5_cal_prefetch_boot`, `tab5_cal_day_tap` ; seul envoi d'une demande de mois = `tab5_cal_request(annee, mois)`, « mois ± 1 » = `cal_shift_month()` (lot 7) | Calendrier | `tab5_calendar.cpp`, `calendar_popup.yaml` |
| `tab5-assist.yaml` | 460 | Assistant vocal complet (lot 8c) : `micro_wake_word` (2 modèles : `okay_nabu` + `Stop` armé/désarmé selon `volet_en_mouvement`), `voice_assistant` (callbacks → `assist_set_pipeline_state()`), image de la réponse (`http_request` + `online_image`), scripts vocaux (`Stop`, interruption, `tab5_wake_word_dispatch`, réponse dans la carte centrale) et du popup Assistant. La pile (avant `script:`) reste sans `lv_*` (règle 2 de `check_tab5_code_rules.py`) | Voix, assistant | `tab5_assist.cpp` (`WakeWord::decide()`), `assistant_popup.yaml` |
| `tab5-lvgl.yaml` | 663 | Layout complet : page unique 1280×720 (`page_main`), swipe gauche/droite = pagination prévisions 0-4 (zone `y ≥ 333` uniquement), console via `btn_control_console` + popup TV via `btn_control_tv`, popup détails plantes via appui long, popup calendrier via appui long horloge, boutons statut/mode vocal (centralisés via `tab5_set_assist_mode`), carte centrale | Layout racine, navigation gestuelle | Tous les `ui_components/*.yaml`, `tab5_central.cpp` (`handle_swipe_gesture`), `g_day_slots`, `g_hour_slots`, `g_central_ctx`) |
| `tab5-alarm.yaml` | 1038 | Réveil matin + annonce des rendez-vous. `rtttl:` (mélodie de sonnerie sur `tab5_speaker`, hors media_player), ~20 entités exposées à HA (`switch`/`datetime type:time`/`number`/`select`/`text`/`text_sensor`/`binary_sensor`/`button`), machine d'état de sonnerie (démarrage, boucle mélodie + 2,5 s de silence, arrêt, répétition, durée max, nettoyage), et un `interval: 1s` qui ne fait qu'UNE comparaison d'entiers (le calcul est mis en cache dans `alarm_clock.cpp`). Point d'entrée unique `script.tab5_alarm_refresh` : entités → `g_alarm_cfg`, jamais l'inverse | Réveil, sonnerie, annonce RDV, entités de réglage HA | `alarm_clock.h/.cpp`, `cal_jours_data[]` (`tab5_custom.h`), IDs LVGL de `ui_components/alarm_popup.yaml` et `alarm_ring_overlay.yaml`, `sntp_time`, `speaker_player`, `micro_wake_word` |

### 3.3 C++ core

| Fichier | Lignes | Rôle exact | Fonctions clés |
|---|---|---|---|
| `tab5_custom.h` | 663 | Déclarations, structs (`CentralPanelCtx` [8 wrappers + le label chapeau du titre de page + 7 flags + current_panel], `DayForecastData`, `HourForecastData`, `WeatherHourSlot`, `WeatherDaySlot`, `MoistureSlotUI`, `PotDetailUI`, `HaAlertSlotUI`, `CalCellUI`, `CalDetailLineUI`), enum `PotMetric`, bits `CAL_BIT_*`, namespace `MeteoIcon::` (codes UTF-8 police météo). Inclut `tab5_tokens.h` | — |
| `tab5_tokens.h` | 116 | Jetons **sans dépendance** (que des `constexpr`, `<cstdint>` seul) : `UIColor::` (palette sémantique — **miroir exact des tokens `color:` YAML, à garder synchro manuellement**), `UIAnim::` (durées/amplitudes), `UIIdle::` (retour à l'accueil). Inclus par `tab5_custom.h` et par les deux seuls jeux qui lisent des couleurs (Arcanoïde, Fil d'Or) ; les six autres jeux n'incluent plus rien du HMI (25/09/2026, lot 8a) | — |
| `tab5_custom.cpp` | 39 | Globals partagés (`g_central_ctx`, `g_day_slots[5]`, `g_hour_slots[5]`, `cal_jours_data`/`cal_heures_data`) et carte des unités. Depuis le 08/09/2026 (lot (e)) la logique vit dans les neuf unités ci-dessous, mêmes fonctions et même ordre que l'ancien fichier de 3 169 lignes ; toutes gardent contre les `lv_obj_t*` nuls | — |
| `tab5_internal.h` | 87 | Les 13 helpers partagés entre unités, **hors contrat YAML**, plus `ui_text()` / `ui_text_color()` (écritures LVGL conditionnelles : rien n'est réécrit ni invalidé si la valeur n'a pas changé, audit du 26/09/2026, lot 3) : texte (`normalize_text_utf8()`, `vigilance_alert_banner_utf8()`, `has_lvgl_recolor_markup()`, `set_label_text_utf8()`) et, depuis le lot 8d (25/09/2026), les 9 fonctions que `tab5_custom.h` déclarait sans qu'aucun YAML ne les appelle (`transition_widgets()`, `close_popup_if_open()`, `animate_swipe_horizontal()`/`_alert_enter()`/`_icon_roll_in()`, `get_temperature_color()`, `tab5_dismiss_local_has/_prune()`, `update_rain_phrase_ui()`). Les helpers d'une seule unité y sont `static` | — |
| `game_common.h` | 372 | Header-only partagé par les **8 consoles** (ADR-0014) : `clampf()`, `xorshift32_next()` (l'état reste dans chaque jeu), `mk_rect()`/`mk_label()`, `show()`/`set_bg()`/`set_border()`/`set_text_if()`/`set_text_color_if()` (sur-ensemble des variantes : gardes nulles, opacité par défaut), `NvsSlot<T>` (clé + magic, `load()`/`save()`/`ready()`) et, depuis le lot 8f (25/09/2026), les mécanismes que 2 à 4 consoles recopiaient : `timer_period_sync()` (tick adaptatif), `topn_insert<>()` (classement), `tilt_calibrate()`/`tilt_smooth()` (inclinaison), `accel_delta_norm()`/`shake_fire()` (secousse) — seuils, périodes et gardes restent dans chaque jeu. Depuis le lot 4 de l'audit des ressources (26/09/2026) : `game_mem_new<T>(MemPref)` / `game_mem_free()` (état d'un jeu pris à l'ouverture, rendu à la fermeture) et `ui_destroy()` (objets LVGL du jeu détruits, conteneurs YAML gardés) — jeu fermé, rien ne reste réservé. Depuis le lot 7 (26/09/2026) : `SlotMenu<N>` + `SlotGeom` (entrées de menu cadre/titre/description : `build()`, `row()`, `hide_from()`, membre du `Mem` de chaque jeu), `set_pressed_bg()`, `show_front()` (calque affiché + premier plan), `hud_num()` (compteur HUD réécrit seulement s'il change). Ne dépend pas du HMI ; un jeu peut redéfinir un helper dans son namespace (`Go::set_bg`) | — |
| `tab5_text.cpp` | 175 | UTF-8 / mojibake, store local des alertes rejetées, `set_label_text_utf8()` (les dates et libellés de jours/mois sont passés dans `tab5_core.cpp`) | voir `tab5_custom.h` |
| `tab5_core.h/.cpp` | 276 | Logique **pure** partagée HMI + réveil (25/09/2026, lot 8b) : `DayForecastData`/`HourForecastData`, `cal_jours_data[]`/`cal_jours_anchor_day` (déclarés ; définis dans `tab5_custom.cpp`), `local_day_from_offset()`/`local_day_number_today()` (seule arithmétique de dates du projet, immunisée aux bascules d'heure), `fr_day_long_utf8()`/`fr_month_long_utf8()`, titres de jour, `cal_is_early_shift()`, `trim_ws()` / `split_fields()` (rognage et découpe « | » en place, lot 7). L'heure passe par `tab5_time_source` (défaut `time`) pour que les tests hôte simulent « maintenant ». Inclus par `tab5_custom.h` | `local_day_from_offset()`, `local_day_number_today()` |
| `tab5_forecast.cpp` | 357 | `update_meteo_icon()`, couleurs température/humidité, `parse_and_update_heures_bulk()`/`_jours_bulk()`, `refresh_daily_forecast()`/`refresh_hourly_forecast()` | voir `tab5_custom.h` |
| `tab5_central.cpp` | 751 | carte centrale (rotateur, `parse_and_update_ha_alerts_bulk()`, `update_info_text_ui()`, titre de page), `handle_swipe_gesture()`/`reset_forecast_to_main_page()`, `show_temporary_planning()`, `show_vocal_response_ui()` | voir `tab5_custom.h` |
| `tab5_services.cpp` | 360 | logique des services HA : `update_volet_ui()`, `parse_and_update_vigilance()`, `update_rain_bar_ui()`, `update_rain_predict_icon_ui()`, `update_clim_from_ha_ui()`, `update_planning_text_ui()` | voir `tab5_custom.h` |
| `tab5_assist.cpp` | 285 | `format_assist_markdown()`, `assist_set_pipeline_state()`, `assist_image_state_ui()`, `WakeWord::decide()` | voir `tab5_custom.h` |
| `tab5_cards.cpp` | 337 | `update_light_card_ui()`, popup lumière, `update_clim_target_ui()`, `sort_and_update_moisture_slots()`, `update_pots_popup_moisture_ui()`/`update_pot_metric_ui()`, `update_temp_ui()` | voir `tab5_custom.h` |
| `tab5_console.cpp` | 192 | ligne status, `ui_sync_volume_widgets()`, `update_console_diagnostics_ui()` | voir `tab5_custom.h` |
| `tab5_anim.cpp` | 551 | `transition_widgets()`, `animate_popup_open(card)` (affichage + premier plan) / `animate_popup_close(card)`, `start_anim()` (lot 7), `animate_swipe_horizontal()`, `animate_alert_enter()`, `ui_idle_ms()`/`close_popup_if_open()`, `animate_icon_roll_in()`, `layout_clock_roller()`/`update_clock_date_ui()`, `setup_button_press_animation()`/`highlight_button_border()` | voir `tab5_custom.h` |
| `tab5_calendar.cpp` | 424 | `cal_grid_build()` (les 42 cellules de la grille, créées à la première ouverture — lot 8), `cal_store_month_data()`, `cal_render_month()`, `cal_render_day_detail()` (cache mensuel `static`) | voir `tab5_custom.h` |
| `tab5_journal.cpp` | 312 | Journal des démarrages et des coupures (26/09/2026) : `logger: on_message` → `journal_log_message()` garde les erreurs (dont `esp-idf` : pilote ESP-Hosted du C6) et, HA déconnecté, les avertissements, en `.noinit` (32 lignes, survit aux resets logiciels et aux plantages, rapport `esp32.crash` d'ESPHome compris) ; `journal_tick()` (interval 30 s) copie en NVS quand le Wi-Fi manque 90 s ; envoi seulement pour un reset anormal, un Wi-Fi absent 90 s, un démarrage sans HA, une erreur après la connexion ou HA joint après 90 s (lignes du démarrage = contexte) ; script `tab5_journal_envoi` (connexion de HA) → événement `esphome.tab5_journal` → garde (e) de `packages/tab5_health.yaml` | voir `tab5_custom.h` |
| `tab5_registry.h/.cpp` | 263 | Registre unique des **consoles** (`GameRegistry::kGames` : libellé, `is_open`, `close`, `on_imu`, `imu_fast`) et des **fenêtres modales** (`ModalRegistry`, genres `POPUP`/`SUBWINDOW`/`LAYER`, rempli une fois par le script `tab5_modal_registry_init` de `tab5-scripts.yaml`). Remplace 4 copies de la liste des jeux et 3 tables de popups (`kPopups` ×2, `kScreens`, `kTargetOf`) — ADR-0013. Garde-fou `tools/check_tab5_registry.py` | `GameRegistry::any_open()` / `open_name()` / `close_all()` / `any_imu_fast_open()` / `dispatch_imu()` ; `ModalRegistry::add()` / `visible_name()` / `any_popup_visible()` / `find()` / `close_all()` |
| `alarm_clock.h/.cpp` | 753 | Moteur du réveil, **pur** : aucun `id()` ESPHome, aucun appel réseau, donc entièrement pilotable depuis un test ou une relecture. Cache de la prochaine sonnerie (recalcul seulement si un réglage/le calendrier a bougé, ou une fois par minute — filet contre bascule d'heure et resynchro SNTP). Toute l'arithmétique de dates passe par `local_day_from_offset()` de `tab5_core.h` (normalisation `mktime()`, immunisée aux bascules heure d'été/hiver). **Ni ESPHome ni LVGL** : testé sur PC par `tools/test_alarm_clock.cpp` (g++ en CI, 13 scénarios) ; le rendu est dans `alarm_render.*` | `alarm_next_ring()` (balaie 8 jours, applique le mode), `ring_minute_for_day()` (les 3 modes + le repos mini dérivé de la FERMETURE de la veille), `alarm_due()` (vrai une seule fois, fenêtre de grâce de 120 s, plancher anti-re-sonnerie sauvegardé en NVS), `alarm_snooze()`/`alarm_dismiss()`, `alarm_next_label()`/`alarm_next_detail()` (annoncent la répétition en cours en priorité), `rdv_store()`/`rdv_due()`/`rdv_next_label()` (appariement par epoch : une re-poussée HA ne ré-annonce pas), `alarm_ring_gain()` (crescendo), `alarm_hhmm()`, `alarm_reset_state()` (tests) |
| `alarm_render.h/.cpp` | 272 | Rendu LVGL du réveil, sorti d'`alarm_clock.cpp` (lot 8b) : ne lit le moteur que par son API publique | `alarm_render_settings()`, `alarm_ring_show/refresh/hide()`, `alarm_render_status_icon()` |
...
|---|---|---|---|
| `climate_card.yaml` | 128 | Carte clim compacte (dashboard principal) | — |
| `climate_popup.yaml` | 307 | Popup clim plein écran (1250×690), 3 cartes de verre : MODE (5 modes empilés), TEMPÉRATURE (arc + cible optimiste + ± débouncés), OPTIONS (Éco/Boost, Silence, Oscillation, Brise `windnice`) | **Partiellement factorisé** (#T164, ADR-0007) : 6/10 boutons via `climate_hvac_mode_btn.yaml`/`climate_preset_toggle_btn.yaml`. Les 4 restants (`off`, `swing`, `windnice`, `quiet`) + les 2 boutons ± volontairement non factorisés (service HA différent par bouton) |
| `climate_hvac_mode_btn.yaml` | 25 | Template paramétré (`!include`+`vars`) pour bouton mode HVAC (342×88) | Template réutilisé 4× |
| `climate_preset_toggle_btn.yaml` | 25 | Template paramétré pour bouton preset eco/boost (164×88) | Template réutilisé 2× |
| `forecast_daily.yaml` | 237 | 5 cartes prévisions journalières (fenêtre glissante sur 15 jours) | Onglets titre/température factorisés via `forecast_day_title_tab.yaml`/`forecast_day_temp_tab.yaml` ; le "corps sombre + action" par carte reste dupliqué 5× (actions HA différentes par carte, cf. §4) |
| `forecast_day_title_tab.yaml` | 14 | Template onglet titre jour | Réutilisé 5× |
| `forecast_day_temp_tab.yaml` | 36 | Template onglet température jour | Réutilisé 5× |
...
| `forecast_hour_card.yaml` | 71 | Template carte horaire individuelle | Réutilisé 5× |
...
| `console_sys.yaml` | 390 | Console Système en 4 cartes de verre : MÉMOIRE (SRAM/PSRAM/bloc max/flash), RÉSEAU (SSID/IP/RSSI/état HA), SYSTÈME (uptime/temp CPU/loop time + volume), GESTION (MAJ écran, reload automations, restart HA, reboot — les 2 derniers derrière un overlay de confirmation) — ouvert via `btn_control_console`, pas par swipe | — |
| `light_popup.yaml` | 200 | Popup contrôle lumière plein écran (1250×690) : sélecteur 3 lumières + On/Off + Tout éteindre, arc luminosité avec % live + raccourcis, 3 blancs + 12 pastilles couleur | Pastilles factorisées via `light_color_preset_btn.yaml` (12 instances), sélecteur via `light_sel_btn.yaml` (3), blancs via `light_white_btn.yaml` (3), raccourcis via `light_level_btn.yaml` (4) — lot 7 ; ouverture/sélection via `script.tab5_light_popup_show` |
| `light_color_preset_btn.yaml` | 24 | Template pastille couleur ronde 78 px | Réutilisé 12× |
| `light_sel_btn.yaml` | 27 | Template ligne du sélecteur de lumière (ids `btn_light_sel_${idx}` / `icon_light_sel_${idx}`, lus par `tab5_light_popup_show`) (lot 7) | Réutilisé 3× |
| `light_white_btn.yaml` | 28 | Template « blanc » nommé (106×92, ampoule teintée + mot, `color_name`) (lot 7) | Réutilisé 3× |
| `light_level_btn.yaml` | 30 | Template raccourci de luminosité (`brightness_pct` + arc et % optimistes) (lot 7) | Réutilisé 4× |
| `tv_remote_popup.yaml` | 329 | Popup télécommande TV Samsung plein écran (1250×690, tokens ADR-0009) : power, pad, volume, chaînes, rangée lecture — `remote.send_command`/`remote.toggle` sur `${entity_tv_remote}` ; ouvert par `btn_control_tv` ou long-press carte PC | Touches via `script.tab5_tv_key` ; flèches `tv_pad_btn.yaml` (4), transport `tv_transport_btn.yaml` (3), applications `tv_app_btn.yaml` (4) — lot 7 |
| `tv_pad_btn.yaml` | 28 | Template flèche du pavé TV (`axis` = nom de clé substitué x/y) (lot 7) | Réutilisé 4× |
| `tv_transport_btn.yaml` | 24 | Template touche de la rangée transport (icône colorée + mot) (lot 7) | Réutilisé 3× |
| `tv_app_btn.yaml` | 26 | Template raccourci d'application (`script.tab5_tv_app` côté HA) (lot 7) | Réutilisé 4× |
| `moisture_sensors.yaml` | 64 | 4 slots UI humidité plantes (tri dynamique sur 5 capteurs BLE) | — |
| `pots_popup.yaml` | 45 | Popup détails plantes plein écran (1250×690) : 5 cartes **fixes** (carte N = capteur `moisture_N`) — nom, icône colorée par humidité, % humidité, statut arrosage + EC/éclairement/température/batterie ; ouvert par appui long sur les pots (`btn_pots_detail_zone`) | Cartes factorisées via `pot_detail_card.yaml` (5 instances) |
| `pot_detail_card.yaml` | 81 | Template carte pot individuelle (226×566, ids paramétrés `${pot_idx}`) | Réutilisé 5× |
| `calendar_popup.yaml` | 192 | Popup calendrier mensuel plein écran (1250×690) : grille 7×6 lundi-en-tête, navigation ◀/▶ + « Aujourd'hui », légende, sous-popup détail jour 780×540 ; grille calculée en local (SNTP), enrichie à la demande par `script.tab5_calendrier_mois`/`_jour` (package HA `tab5_calendar.yaml`) ; ouvert par appui long sur l'horloge (`btn_clock_calendar_zone`) | Cellules construites en C++ par `cal_grid_build()` juste avant `cal_legend` (lot 8, 26/09/2026 : les 42 `!include` de l'ancien `cal_day_cell.yaml` coûtaient ≈ 45 Ko de `setup()`) |
| `arcade_card.yaml` | 37 | Template carte de la page Arcade (298×252) : `game`, `x`, `y`, `bg`, `icon`, `title`, `subtitle` (lot 8e) | Réutilisé 8× par `game_selector.yaml` |
| `ha_alert_panel.yaml` | 35 | Template bandeau d'alerte/info HA de la carte centrale (panneaux 4-7), ids `${n}` (lot 8e) | Réutilisé 4× par `tab5-lvgl.yaml` |
| `alarm_day_chip.yaml` | 29 | Template pastille de jour du réveil (72×52) : `n`, `x`, `letter` (lot 8e) | Réutilisé 7× par `alarm_popup.yaml` |
| `alarm_step_script_btn.yaml` | 33 | Template bouton à pas du réveil : `call` = mapping remis tel quel à `script.execute` (`{ id: tab5_alarm_shift, cible: 0, delta: 60 }`, `{ id: tab5_alarm_cycle_melody, delta: 1 }`) (lot 8e, `call` depuis le lot 7) | Réutilisé 10× par `alarm_popup.yaml` |
| `alarm_step_number_btn.yaml` | 26 | Template bouton −/+ d'une entité `number` du réveil (`number.${op}`, `cycle: false`) (lot 8e) | Réutilisé 10× par `alarm_popup.yaml` |
| `assistant_popup.yaml` | 343 | Popup assistant vocal plein écran : colonne réglages (cerveau Domotique/Discussion, Ok Nabu, muet, volume, taille A-/A+ en roboto_32_b / roboto_45_b) + transcription STT + réponse Markdown (plus de monospace depuis le 26/09/2026) + image `online_image` à la demande (`tab5_assist_reponse`) | — |
| `alarm_popup.yaml` | 420 | Fenêtre de réglage du réveil (colonne gauche = quand ça sonne, droite = ce qui se passe), positions absolues, **0 hex, aucune logique** : chaque geste appelle un script de `tab5-alarm.yaml`, tout le repaint passe par `alarm_render_settings()` | Modèle de référence (règles 1 et 2 respectées à 100 %) |
| `alarm_ring_overlay.yaml` | 96 | Calque plein cadre de sonnerie (heure, titre, sous-titre, bouton Répéter — le voile partagé EST le bouton Arrêter), déclaré en **dernier** dans `tab5-lvgl.yaml` (ordre de dessin LVGL) | — |
...
| Fichier | Rôle |
|---|---|
| `.github/workflows/esphome-tab5.yml` | CI GitHub Actions (PR + push sur `main`) : job `build-revisions` (matrice `st7121`/`ili9881c` : `tab5_ecran:` ajouté au `user_entities.yaml` factice, cache de `build` restauré sans sauvegarde, non requis, lancé seulement si l'écran/le matériel/l'entrée/le workflow changent) ; job `python` (pre-commit, pytest, moteur Go C++ par g++, dry-run démo) et job `build` (`secrets.yaml` factice, `esphome/build-action@v8.1.0` sur `latest`, ccache conservé entre runs, sauté si seuls des `.md` changent) et job `build-min` (même compilation avec la version lue dans `min_version:` de `tab5-ha-hmi.yaml`, cache ccache à part, sans artefact) ; artefact `tab5-firmware` sur `main`. `python`, `build` et `build-min` sont requis |
| `README.md` (racine) | Doc utilisateur bilingue EN/FR. Premier écran (26/09/2026) : ce que c'est, GIF, « Why this one », « Before you start » (limites honnêtes : ST7123, pas de binaire, textes FR, disposition de la maison de l'auteur), quick start, compatibilité matérielle. Puis zones fonctionnelles, choix d'architecture, carte de la documentation, communauté ; la note personnelle est juste avant la note sur l'IA |
| `CONTRIBUTING.md`, `CODE_OF_CONDUCT.md`, `SECURITY.md` | Fichiers communautaires bilingues (profil communautaire GitHub). Le code de conduite est adapté du Contributor Covenant 2.1 ; les signalements (conduite, failles) passent en privé, jamais d'adresse e-mail écrite dans le dépôt |
| `.github/ISSUE_TEMPLATE/` | Formulaires d'issue bilingues : `bug_report.yml` (puce écran du Tab5 demandée en premier, version d'ESPHome et du firmware), `feature_request.yml`, `config.yml` (pas d'issue libre ; questions, retours matériels et montages → Discussions ; failles → politique de sécurité) · `.github/PULL_REQUEST_TEMPLATE.md` : checklist des PR |
| `Tab5/README.md` | Description fichier par fichier + table des services API (16) + table des globals + 8 règles de code ; les jeux sont dans `docs/arcade.md` depuis le 25/09/2026 |
| `docs/*.md` (12 fichiers) | `architecture.md`, `hardware.md`, `ui_design.md`, `voice_assistant.md`, `installation.md`, `screens.md`, `demo_mode.md`, `troubleshooting.md`, `debugging.md`, `related_projects.md`, `arcade.md` (les 8 consoles), `INVENTAIRE_CONFIGS_TESTS.md` + `docs/decisions/` (ADR) + `docs/press/` (kit de publication Hackster) (17 ADR) + `docs/images/`. `hardware.md` s'ouvre sur le tableau des révisions du Tab5 (ST7123 prise en charge, ST7121 et ILI9881C pas encore). `docs/` ne contient que de la doc de référence : les anciens rapports d'audit LLM (synthétisés en #T161-#T169, cf. `audit_tab5/` côté workspace privé), les patchs de travail et les sauvegardes d'essais en ont été retirés |

---

## 4. Points de friction / dette technique

`maj: 2026-09-06` · Classés par impact décroissant. Audit complet le 06/09/2026 : P0 traités dans la PR #100, P1/P2 (dépôt, refactos firmware, CI) listés dans `contexte_ia/audits_systeme/audit_tab5_2026-09-06.md` (workspace privé, hors dépôt). Points résolus depuis la rédaction initiale (06/07) sont barrés ou déplacés en §4.6.
...
- ~~**Callbacks `voice_assistant:` recopiés 5× et `on_wake_word_detected` à 5 niveaux d'if/else**~~ — **RÉSOLU** (08/09/2026, lot (c)) : `assist_set_pipeline_state(AssistState)` / `assist_set_mic_state()`, `WakeWord::decide(Inputs)` (table à 6 sorties) + script `tab5_wake_word_dispatch`, `assist_image_state_ui(AssistImage)`, `assist_wake_word_indicator_ui()` ; `tab5-hardware.yaml` sans aucun `lv_*`, garde-fou en CI.
- ~~**Logique LVGL dans `tab5-api-logic.yaml`** malgré son en-tête et ADR-0006 (volet ~50 lignes, vigilance ~80, pluie 1 h, probabilités, clim)~~ — **RÉSOLU** (08/09/2026, lot (a)) : `update_volet_ui()`, `parse_and_update_vigilance()`, `update_rain_bar_ui()`, `update_rain_predict_icon_ui()`, `update_clim_target_ui()`, `assist_image_hint_ui()` dans `tab5_custom.cpp` ; le contrat ne contient plus que des `id()` + appels, garde-fou en CI.
...
- **`pressed: bg_opa: 30%` répété sur les boutons verre** — `pressed:` est refusé dans `style_definitions:`, mais **accepté sur un widget sous la forme `pressed: { styles: style_x }`** (vérifié le 26/09/2026) : ces répétitions peuvent donc être partagées (point D6 de l'audit ressources, pas encore fait).
...
- ~~**`tab5_custom.cpp` (3169 lignes au 08/09/2026)** — 7 responsabilités ; découpage en unités de compilation recommandé par l'audit du 06/09~~ — **RÉSOLU** (08/09/2026, lot (e)) : neuf unités `tab5_*.cpp` (3335 lignes, de 164 à 741), `tab5_custom.h` inchangé comme unique en-tête public, `tab5_internal.h` pour les six helpers partagés ; preuve de neutralité = diff des `esphome config` limité à la liste `includes:`.
